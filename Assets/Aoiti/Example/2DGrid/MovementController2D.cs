...
public class MovementController2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (pathLeftToGo.Count > 0) //if the target is not yet reached
        {
            ...
            transform.position += dir.normalized * deltaTimeSpeed;
            if (((Vector2)transform.position - pathLeftToGo[0]).sqrMagnitude < deltaTimeSpeed * deltaTimeSpeed) 
            {
    ...
    private void OnDrawGizmos()
    {
        ...
        foreach (var next in pathLeftToGo)
        {
